...
function Navbar() {
  return (
    <div
      id="Navbar"
      className=" w-full h-[50px] px-6 pt-4 flex justify-between items-center"
    >
      <motion.div
      initial={{x:-200}}
      animate={{x:0}}
      ...
      <motion.div
        id="links"
        className="flex gap-9 font-[NeueMontreal,Roboto,sans-serif] font-normal text-[16px] text-[#212121] w-fit h-fit  "
      >
        {["Services", "Our work", "About", "Insight", "Contact us"].map(
          (link, index) => {
            return (
              <motion.a
              initial={{opacity:0}}
              animate={{opacity:100}}
              transition={{duration:2, delay:index*.2,ease:[0.76, 0, 0.24, 1]}}
                key={index}
                href="2"
                className={`text-[1.4vw]  relative   after:absolute after:w-full after:h-[2px] after:bg-black after:left-0 after:bottom-0 after:rounded-full  after:scale-x-1 after:duration-300 after:origin-right after:transition after:scale-x-0  hover:after:origin-left hover:after:scale-x-100 ${
                  index === 4 ? "ml-[200px]" : null
                } `}
              >
                {link}
              </motion.a>
